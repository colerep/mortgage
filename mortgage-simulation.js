...
function simulateArmRates(initialRate, margin, initialCap, annualCap, lifetimeCap, years) {
    ...
    if (annualRateChanges.length > 0) {
        ...
        for (let year = 0; year < numRemainingYears; year++) {
            // Sample a random annual change from historical data
            const randomIndex = Math.floor(Math.random() * annualRateChanges.length);
            const annualChange = annualRateChanges[randomIndex];
            
            // Apply the sampled change to the index rate
            currentIndexRate += annualChange;
            currentIndexRate = Math.max(0.5, currentIndexRate); // Floor at 0.5%
